...
begin
  ...
  display_name := nullif(trim(coalesce(meta->>'display_name', meta->>'full_name', meta->>'name', '')), '');
  user_type := lower(nullif(trim(coalesce(meta->>'user_type', 'buyer')), ''));

  if full_name is null then
    full_name := 'User ' || substr(new.id::text, 1, 5);
  end if;
