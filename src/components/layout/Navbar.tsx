...
export default function Navbar() {
  ...
  const roleFromUserMeta = (authUser as any)?.user_metadata?.role
    ? String((authUser as any).user_metadata.role).toLowerCase()
    : "";
  const roleFromAppMeta = (authUser as any)?.app_metadata?.role
    ? String((authUser as any).app_metadata.role).toLowerCase()
    : "";
  const cachedRole = (() => {
    if (!authUser?.id) return "";
    try {
      return String(localStorage.getItem(`smp:role:${authUser.id}`) || "");
    } catch {
      return "";
    }
  })();

  const role = String(roleFromProfile || roleFromUserMeta || roleFromAppMeta || cachedRole || "").toLowerCase();

  // Persist role once known (keeps Admin menu visible even during refresh/profile loading)
  try {
  ...
  const accountLabel = !signedIn
    ...
    ? isAdmin
      ...
      : "Buyer"
    : "Account";
  const planTier = normalizeTier((profile as any)?.membership_tier ?? "free");
  const planLabel =
    planTier === "premium"
      ? "Premium"
      : planTier === "pro"
  ...
  // Ã¢Å“â€¦ NEW: Desktop account dropdown state
  ...
  const topItems: NavItem[] = useMemo(() => {
    if (!signedIn) return [];
    return [
      { label: "Marketplace", path: "/marketplace", show: true },
  ...
  const displayName = (businessName || profileName || metaName || emailFallbackName || emailName || "Account").trim();
  ...
  const handleSignUp = () => {
    ...
    console.log("AUTH_UI: open signup");
    setSignupError(null);
    const hasOpener =
      typeof (window as any).openRegisterModal === "function" ||
      typeof (window as any).smpOpenAuth === "function";
    try {
      openAuth("register");
    } catch {
      setSignupError("Could not open sign up. Please refresh.");
      return;
    }
    if (!hasOpener) {
      setSignupError("Could not open sign up. Please refresh.");
          ...
          <div className="flex items-center gap-2">
            {!signedIn ? (
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={() => navigateToPath("/pricing")}
                  className="text-sm font-semibold px-3 py-2 rounded-lg text-slate-700 hover:text-brand hover:bg-slate-50"
                >
                  Upgrade plan
                </button>
                ...
                {signupError ? (
                  <div className="text-xs text-rose-600">{signupError}</div>
                ) : null}
              </div>
            ) : (
              <>
                ...
                {/* Ã¢Å“â€¦ Desktop Account Dropdown */}
                    ...
                    <div className="absolute right-0 mt-2 w-56 rounded-2xl border border-slate-200 bg-white shadow-lg p-1">
                      {!profileReady ? (
                        <>
                          ...
                          <div className="px-3 py-2 text-xs font-semibold text-slate-500">Plan: Loading...</div>
              ...
              <div className="p-2 grid gap-1">
                ...
                {topItems.map((it) => (
                  ...
                  >
                    ...
                    {it.path === "/marketplace" && <Icon name="grid" />}
                  </button>
                ))}
                ...
                <button
                  ...
                  className={cn(
                    "w-full text-left px-3 py-3 rounded-xl text-sm font-semibold flex items-center justify-between",
                    path === "/help" ? "bg-slate-50 text-brand" : "hover:bg-slate-50 text-slate-800"
                  )}
                >
                ...
                {profileReady && !isSeller && !isAdmin && (
                  <button
                    type="button"
                    onClick={handleBecomeSeller}
                    className={cn(
                      "w-full text-left px-3 py-3 rounded-xl text-sm font-semibold flex items-center justify-between",
                      path === "/seller/setup" ? "bg-slate-50 text-brand" : "hover:bg-slate-50 text-slate-800"
                    )}
                  >
